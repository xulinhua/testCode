...
def generate_launch_description():
    ...
    cam_id_arg = DeclareLaunchArgument(
        'camera_id',
        default_value='',
        description='Camera ID to use for elevator recognition'
    )
    ...
    # 创建节点的函数
    def create_elevator_rec_node(context):
        ...
        node_name = f'elevator_rec_node_{camera_id}'
        ...
        elevator_rec_node = Node(
            package='elevator_rec',
            executable='elevator_rec',
            name=node_name,
            output='screen',
            parameters=[params_dict]
        )
        return [elevator_rec_node]

    # 使用OpaqueFunction来动态创建节点
    elevator_rec_node_creator = OpaqueFunction(function=create_elevator_rec_node)
    ...
    ld.add_action(elevator_rec_node_creator)
    return ld
