...
def generate_launch_description():
    ...
    cam_id_arg = DeclareLaunchArgument(
        'camera_id',
        default_value='',
        description='Camera ID to use for cassette detection'
    )
    ...
    # 创建节点的函数
    def create_cassette_det_node(context):
        ...
        node_name = f'cassette_det_node_{camera_id}'
        ...
        cassette_det_node = Node(
            package='cassette_det',
            executable='cassette_det',
            name=node_name,
            output='screen',
            parameters=[params_dict]
        )
        return [cassette_det_node]

    # 使用OpaqueFunction来动态创建节点
    cassette_det_node_creator = OpaqueFunction(function=create_cassette_det_node)
    ...
    ld.add_action(cassette_det_node_creator)
    return ld
