...
def generate_launch_description():
    ...
    camera_id_arg = DeclareLaunchArgument(
        'camera_id',
        default_value='',
        description='Camera ID to use for gesture recognition'
    )
    ...
    # 创建节点的函数
    def create_gesture_rec_node(context):
        ...
        node_name = f'gesture_rec_node_{camera_id}'
        ...
        gesture_rec_node = Node(
            package='hand_gesture_rec',
            executable='hand_gesture_rec',
            name=node_name,
            output='screen',
            parameters=[params_dict]
        )
        return [gesture_rec_node]

    # 使用OpaqueFunction来动态创建节点
    gesture_rec_node_creator = OpaqueFunction(function=create_gesture_rec_node)
    ...
    ld.add_action(gesture_rec_node_creator)
    return ld
