...
def generate_launch_description():
    ...
    cam_id_arg = DeclareLaunchArgument(
        'camera_id',
        default_value='',
        description='Camera ID to use for object segmentation'
    )
    ...
    # 创建节点的函数
    def create_object_seg_node(context):
        ...
        node_name = f'object_seg_node_{camera_id}'
        ...
        object_seg_node = Node(
            package='object_seg',
            executable='object_seg',
            name=node_name,
            output='screen',
            parameters=[params_dict]
        )
        return [object_seg_node]

    # 使用OpaqueFunction来动态创建节点
    object_seg_node_creator = OpaqueFunction(function=create_object_seg_node)
    ...
    ld.add_action(object_seg_node_creator)
    return ld
