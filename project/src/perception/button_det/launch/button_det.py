...
def generate_launch_description():
    ...
    cam_id_arg = DeclareLaunchArgument(
        'camera_id',
        default_value='',
        description='Camera ID to use for button detection'
    )
    ...
    # 创建节点的函数
    def create_button_det_node(context):
        ...
        node_name = f'button_det_node_{camera_id}'
        ...
        button_det_node = Node(
            package='button_det',
            executable='button_det',
            name=node_name,
            output='screen',
            parameters=[params_dict]
        )
        return [button_det_node]

    # 使用OpaqueFunction来动态创建节点
    button_det_node_creator = OpaqueFunction(function=create_button_det_node)
    ...
    ld.add_action(button_det_node_creator)
    return ld
