    ...
    cudaStream_t stream)
{
    ...
    // Copy keep count to host
    int keep_count = 0;
    cudaMemcpyAsync(&keep_count, d_keep_num, sizeof(int), cudaMemcpyDeviceToHost, stream);
    ...
    int* d_keep = nullptr;
    cudaMallocAsync(&d_keep, keep_count * sizeof(int), stream);
    
    nms_kernel<<<grid, block, 0, stream>>>(d_out, keep_count, iou_thresh, d_keep);
    
    // Copy results to host
    std::vector<Bbox32> host_boxes(keep_count);
    cudaMemcpyAsync(host_boxes.data(), d_out, keep_count * sizeof(Bbox32), 
                    cudaMemcpyDeviceToHost, stream);
    std::vector<int> host_keep(keep_count);
    cudaMemcpyAsync(host_keep.data(), d_keep, keep_count * sizeof(int),
                    cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
...
// Kernel: 使用1D grid + while循环处理bbox内的像素
    ...
    cudaStream_t stream)
{
    ...
    const int THREADS_PER_BLOCK = 1024;  // 增加到1024以支持大bbox并行
