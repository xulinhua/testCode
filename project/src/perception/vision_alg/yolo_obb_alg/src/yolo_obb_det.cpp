...
// 旋转框IOU计算函数
static float calculate_obb_iou(const Detection& det1, const Detection& det2) {
    // 获取两个检测框的参数
    float cx1 = det1.bbox[0], cy1 = det1.bbox[1], w1 = det1.bbox[2], h1 = det1.bbox[3], angle1 = det1.bbox[4];
    float cx2 = det2.bbox[0], cy2 = det2.bbox[1], w2 = det2.bbox[2], h2 = det2.bbox[3], angle2 = det2.bbox[4];
    
    // 计算旋转框的四个顶点
    auto get_rotated_vertices = [](float cx, float cy, float w, float h, float angle) -> std::vector<cv::Point2f> {
        std::vector<cv::Point2f> vertices(4);
        float cos_a = cos(angle);
        float sin_a = sin(angle);
        
        vertices[0] = cv::Point2f(cx + w/2*cos_a - h/2*sin_a, cy + w/2*sin_a + h/2*cos_a);
        vertices[1] = cv::Point2f(cx - w/2*cos_a - h/2*sin_a, cy - w/2*sin_a + h/2*cos_a);
        vertices[2] = cv::Point2f(cx - w/2*cos_a + h/2*sin_a, cy - w/2*sin_a - h/2*cos_a);
        vertices[3] = cv::Point2f(cx + w/2*cos_a + h/2*sin_a, cy + w/2*sin_a - h/2*cos_a);
        
        return vertices;
    };
    
    // 获取两个旋转框的顶点
    std::vector<cv::Point2f> vertices1 = get_rotated_vertices(cx1, cy1, w1, h1, angle1);
    std::vector<cv::Point2f> vertices2 = get_rotated_vertices(cx2, cy2, w2, h2, angle2);
    
    // 计算相交面积
    std::vector<cv::Point2f> intersection;
    cv::rotatedRectangleIntersection(
        cv::RotatedRect(cv::Point2f(cx1, cy1), cv::Size2f(w1, h1), angle1 * 180 / CV_PI),
        cv::RotatedRect(cv::Point2f(cx2, cy2), cv::Size2f(w2, h2), angle2 * 180 / CV_PI),
        intersection
    );
...
void YoloObbDet::load_engine(const std::string& engine_path) 
{
    //检查文件是否存在
    ...
    // 对于OBB检测，每个检测框有5个参数（cx, cy, w, h, angle）
    num_classes_ = output_dims.d[1] - 5;
...
void YoloObbDet::prepare_buffers() {
    // 分配输入输出内存
...
void YoloObbDet::preprocess(const cv::Mat& img) 
{
    cv::Mat resized;
...
std::vector<Detection> YoloObbDet::infer(cv::Mat& img) 
{
    try 
    {
        bool bTestTimes = false; 
...
void YoloObbDet::postprocess(std::vector<Detection>& detections, int img_width, int img_height) 
{
    // 1. GPU → CPU
    ...
    for (int i = 0; i < num_boxes_; ++i) 
    {
        ...
        float angle = output[(num_classes_ + 4) * num_boxes_ + i];  // 角度信息
    ...
    if (!detections.empty()) 
    {
        ...
        for (size_t i = 0; i < detections.size(); ++i) 
        {
            ...
            for (size_t j = i + 1; j < detections.size(); ++j) 
            {
                ...
                // 对于OBB检测，使用旋转框的IOU计算
                float iou = calculate_obb_iou(detections[i], detections[j]);
...
void YoloObbDet::draw_results(cv::Mat& img, 
                          ...
                          const std::vector<std::string>& class_names) 
{
    ...
    for (const auto& det : detections) 
    {
        ...
        // 绘制旋转框
        float angle = det.bbox[4]; // 角度信息
        float opencv_angle = angle * 180 / CV_PI;
        if (opencv_angle >= 90.0f) {
            opencv_angle = opencv_angle -90.0;
        } 
        cv::RotatedRect rotated_rect(cv::Point2f(det.bbox[0], det.bbox[1]), 
                                   cv::Size2f(det.bbox[2], det.bbox[3]), 
                                   opencv_angle);
        ...
        for (int i = 0; i < 4; i++) {          
            cv::line(img, vertices[i], vertices[(i+1)%4], color, 2);//崩溃
        }
        ...
        int baseline;
        cv::Size text_size = cv::getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &baseline);//崩溃
        // 调整标签框的位置，确保不超出图像顶部
        int text_x = std::max(0, (int)det.bbox[0] - text_size.width / 2);
        int text_y = std::max(text_size.height + 5, (int)det.bbox[1]);
        cv::rectangle(img, 
                     cv::Point(text_x, text_y - text_size.height - 5),
                     cv::Point(text_x + text_size.width, text_y),
                     color, cv::FILLED);
        cv::putText(img, label, 
                   cv::Point(text_x, text_y - 5),
                           ...
                           const std::vector<std::string> &class_names)
{
    ...
    for (const auto& det : detections) 
    {
        ...
        // 获取深度值（单位：毫米）
        int pixel_x = static_cast<int>(det.bbox[0]);
        int pixel_y = static_cast<int>(det.bbox[1]);
        ...
        */
        ...
        // std::cout << class_names[det.class_id] << ":xPos,yPos,zPos,angle :" << X << "," << Y << "," << Z << "," << angle * 180 / CV_PI <<std::endl;
...
bool YoloObbDet::IsStairs(const Detection& detection, const cv::Mat& depth_img)
{   
    // 将中心点坐标转换为左上角坐标
...
void YoloObbDet::FilterRes(const cv::Mat& depth_img, std::vector<Detection>& detections)
{
...
void YoloObbDet::preprocess_gpu(const cv::Mat& img)
{
    int src_h = img.rows;
    int src_w = img.cols;
...
void YoloObbDet::postprocess_gpu(std::vector<Detection>& detections,
                              int img_w, int img_h)
{
    // 申请 GPU 缓存（只申请一次）
