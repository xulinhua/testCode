...
def generate_launch_description():
    ...
    cam_id_arg = DeclareLaunchArgument(
        'camera_id',
        default_value='',
        description='Camera ID to use for Face detection'
    )
    ...
    # 创建节点的函数
    def create_face_detection_node(context):
        ...
        node_name = f'face_detection_node_{camera_id}'
        ...
        return [face_det_node]

    # 使用OpaqueFunction来动态创建节点
    face_detection_node_creator = OpaqueFunction(function=create_face_detection_node)
    ...
    ld.add_action(face_detection_node_creator)
    return ld
