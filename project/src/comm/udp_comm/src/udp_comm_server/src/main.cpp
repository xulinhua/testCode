...
int main(int argc, char** argv)
{
  ...
  std::string default_config_file_path = ament_index_cpp::get_package_share_directory("udp_comm_server") + "/config" + "/udp_server_params.yaml";
  if (argc <= 1)
  ...
  auto node = std::make_shared<udp_comm_server::UDPServerNode>();
