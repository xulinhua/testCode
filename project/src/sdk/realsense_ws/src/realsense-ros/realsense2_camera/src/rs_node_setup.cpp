...
void BaseRealSenseNode::stopPublishers(const std::vector<stream_profile>& profiles)
{
    for (auto& profile : profiles)
    {
        ...
        if (profile.is<rs2::video_stream_profile>())
        {
            ...
            if (_compressed_image_publishers.find(sip) != _compressed_image_publishers.end())
            {//清理压缩发布器
                _compressed_image_publishers[sip].shutdown();
                _compressed_image_publishers.erase(sip);
            }
...
void BaseRealSenseNode::startPublishers(const std::vector<stream_profile>& profiles, const RosSensor& sensor)
{
    ...
    for (auto& profile : profiles)
    {
        ...
        if (profile.is<rs2::video_stream_profile>())
        {
            ...
            // 创建压缩图像发布器
            if (_enable_compressed_image)
            {
                try
                {
                    // 获取底层节点指针
                   auto node_ptr = _node.shared_from_this();
                    
                    // 创建image_transport实例
                    image_transport::ImageTransport it(std::static_pointer_cast<rclcpp::Node>(node_ptr));
                    
                    // 使用独立的话题名称，不与原始图像话题嵌套
                    std::string compressed_topic = "~/" + stream_name + "/image_compressed";
                    
                    // 直接创建Publisher对象，不使用shared_ptr包装
                    _compressed_image_publishers[sip] = it.advertise(compressed_topic, 1);
                    
                    ROS_INFO_STREAM("Created compressed image publisher for topic: " << compressed_topic);
                }
                catch (const std::exception& e)
                {
                    ROS_ERROR_STREAM("Failed to create compressed image publisher: " << e.what());
                }
            }
