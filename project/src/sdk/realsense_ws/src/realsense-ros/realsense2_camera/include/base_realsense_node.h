...
namespace realsense2_camera
{
    ...
    private:
        ...
        std::map<stream_index_pair, image_transport::Publisher> _compressed_image_publishers; // 压缩图像发布器映射
        //std::map<stream_index_pair, std::shared_ptr<image_transport::Publisher>> _compressed_image_publishers;// 压缩图像发布器映射 
        bool _enable_compressed_image;// 是否启用压缩话题的参数
