...
class MonocularCameraNode(Node):
    def __init__(self):
        ...
        self.publisher_raw = self.create_publisher(
            Image, "/csi_camera_0/color/image_raw", 10
        )  # 创建原始图像发布者
        self.publisher_compressed = self.create_publisher(
            Image, "/csi_camera_0/image_compressed", 10
        )  # 创建压缩图像发布者

        self.publisher_camera_info = self.create_publisher(
            CameraInfo, "/csi_camera_0/color/camera_info", 10
        )
    ...
    def load_camera_info(self):
        ...
        if self.calibration_file_path is None:
            ...
            possible_paths = [
                ...
                os.path.expanduser("~/.config/csi_cam_service/default_calibration.yaml"),
                os.path.join(os.path.dirname(__file__), "..", "..", "config", "default_calibration.yaml")
            ]
            
            for path in possible_paths:
                if os.path.exists(path):
                    self.calibration_file_path = path
                    self.get_logger().info(f"Auto-loading calibration file: {path}")
                    break
        
        if self.calibration_file_path is not None and os.path.exists(self.calibration_file_path):
            ...
            self.get_logger().warn(f"Calibration file not found: {self.calibration_file_path}")
            # 如果找不到校准文件，创建一个基本的CameraInfo消息
            camera_info_msg = CameraInfo()
            camera_info_msg.height = self.height
            camera_info_msg.width = self.width
            camera_info_msg.header.frame_id = "camera"
            self.get_logger().warn("Using basic CameraInfo without calibration data")
    ...
    def publish_camera_calibration(self):
        if self.calibration_data:
            # 更新时间戳
            self.calibration_data.header.stamp = self.get_clock().now().to_msg()
            # Publish camera calibration parameters
            self.publisher_camera_info.publish(self.calibration_data)
            self.get_logger().debug("Camera info published")
