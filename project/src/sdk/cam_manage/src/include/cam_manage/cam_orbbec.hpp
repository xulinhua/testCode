#ifndef CAMERA_ORBBEC
#define CAMERA_ORBBEC
...
class CamOrbbec : public CamBase
{
public:
	CamOrbbec();
	virtual ~CamOrbbec();
	...
	static std::shared_ptr<ob::DeviceList> device_list_; // 静态设备列表
	static std::shared_ptr<ob::Context> context_; // 静态Context对象
	...
	// 图像获取
	virtual RtnType get_one_frame(CamFramelist &frames); // 获取一帧图像
...
private:
	...
	std::shared_ptr<ob::Config> config_;	// 状态标志
