#ifndef CAM_MGR_ROS__CAM_RS_HPP_
#define CAM_MGR_ROS__CAM_RS_HPP_
...
/**
 * @brief RealSense相机管理类
 */
class CamRs : public CamBase
{
public:
    CamRs();
    ~CamRs();
...
public:
    ...
    /**
     * @brief 停止指定 RealSense相机 ID 的相机进程
     * @param cam_id 相机 ID
     * @param camera_state 相机状态信息（引用参数）
     * @return 是否停止成功
     */
    ...
    /**
     * @brief 获取RealSense相机的ROI和FPS列表（静态函数）
     * @param cam_id 相机ID
     * @param sensor_roi_fps_map 传感器ROI和FPS列表输出参数
     * @param cam_info 相机配置信息（引用参数）
     * @return 是否获取成功
     */
    ...
    /**
     * @brief 枚举RealSense相机设备（静态函数）
     * @param cam_dev_list RealSense相机设备信息列表输出参数
     * @return 是否枚举成功
     */
...
#endif  // CAM_MGR_ROS__CAM_RS_HPP_