...
public:
	...
	platform_critical_section GetLock() { return cs_Dat; }
	bool LoadImage(std::string path);
	bool SaveImage(std::string path);
private:
	cv::Mat* pMat;
	platform_critical_section cs_Dat;//如果在多线程中调用不加锁会引起崩溃，默认在类的外部加锁防护
};

HSV_CUSTRCT_API void ClearGCImage(GCImage** img);