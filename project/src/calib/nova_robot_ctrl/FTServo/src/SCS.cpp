...
int	SCS::Ping(u8 ID)
{
	...
	//std::cout<<"SCS::Ping"<<INST_PING<<std::endl;
	wFlushSCS();
	Error = 0;
	...
	//std::cout<<"SCS::Ping_0"<<bBuf[0]<<" "<<bBuf[1]<<" "<<bBuf[2]<<" "<<bBuf[3]<<" "<<bBuf[4]<<" "<<bBuf[5]<<"Size:"<<Size<<std::endl;
	if(Size!=6){
		return -1;
	}
	//std::cout<<"SCS::Ping_1"<<std::endl;
	if(bBuf[0]!=0xff || bBuf[1]!=0xff){
		return -1;
	}
	//std::cout<<"SCS::Ping_2"<<std::endl;
	if(bBuf[2]!=ID && ID!=0xfe){
		return -1;
	}
	//std::cout<<"SCS::Ping_3"<<std::endl;
	if(bBuf[3]!=2){
		return -1;
	}
	//std::cout<<"SCS::Ping_4"<<std::endl;
	for(i=2; i<(Size-1); i++){
		calSum += bBuf[i];
	}
	calSum = ~calSum;
	//std::cout<<"SCS::Ping_5"<<std::endl;
	if(calSum!=bBuf[Size-1]){
		return -1;
	}
	Error = bBuf[2];
	//std::cout<<"SCS::Ping_6"<<std::endl;
	return Error;
}
