...
{
	//std::cout<<"SCSerial::begin called with baudRate: "<<baudRate<<" and serialPort: "<<serialPort<<std::endl;
	if(fd != -1){
		close(fd);
		fd = -1;
	}
	//printf("servo port:%s\n", serialPort);
    //std::cout<<"SCSerial::begin 1"<<std::endl;
    if(serialPort == NULL)
		return false;
    //std::cout<<"SCSerial::begin 2"<<std::endl;
    fd = open(serialPort, O_RDWR | O_NOCTTY | O_NONBLOCK);
    if(fd == -1){
		perror("open:");
        return false;
	}
    //std::cout<<"SCSerial::begin 3"<<std::endl;
    fcntl(fd, F_SETFL, FNDELAY);
    tcgetattr(fd, &orgopt);
    tcgetattr(fd, &curopt);
    //std::cout<<"SCSerial::begin 4"<<std::endl;
    speed_t CR_BAUDRATE;
    switch(baudRate){
    case 9600:
        CR_BAUDRATE = B9600;
    ...
    case 1000000:
        ...
        break;
    default:
		//std::cout<<"SCSerial::begin using default baud rate for value: "<<baudRate<<std::endl;
		CR_BAUDRATE = B115200;
        break;
    }
    ...
    //std::cout<<"SCSerial::begin 5"<<std::endl;
    try
    {
        if(tcsetattr(fd, TCSANOW, &curopt) == 0)
        {
			//std::cout<<"SCSerial::begin ok"<<std::endl;
            return true;
		}
         else
         {
		perror("tcsetattr:");
        //std::cout<<"SCSerial::begin ng"<<std::endl;
		return false;
	    }
    }
    catch(...)
    {
		//std::cout<<"SCSerial::begin exception caught"<<std::endl;
    }
    //std::cout<<"SCSerial::begin 6"<<std::endl;
    return false;
}
