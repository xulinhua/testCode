...
# 全局变量，控制是否获取指定用户坐标系和工具坐标系的笛卡尔坐标
# 若为False，则使用默认的基坐标系和默认工具坐标系
USE_CUSTOM_COORDINATE_SYSTEM = False


class Nova2Robot:
    """越疆 Nova2 机械臂控制类（模块化封装版）"""
    
    def __init__(self, ip_address: str = "192.168.5.1", user_coordinate_id: int = 0, tool_coordinate_id: int = 0):
        """初始化 Nova2 机械臂
        
        Args:
            ip_address: 机械臂IP地址，默认为"192.168.5.1"
            user_coordinate_id: 用户坐标系ID，默认为0（基坐标系）
            tool_coordinate_id: 工具坐标系ID，默认为0（默认工具）
        """
        self.ip_address = ip_address
        self.user_coordinate_id = user_coordinate_id
        self.tool_coordinate_id = tool_coordinate_id
        self.user_coordinate_id = 0
        self.tool_coordinate_id = 2
        self.sock_29999 = None
        self.is_connected = False
        self.logger = logging.getLogger(self.__class__.__name__)
        
        # 打印坐标系ID信息
        print(f"用户坐标系ID: {self.user_coordinate_id}")
        print(f"工具坐标系ID: {self.tool_coordinate_id}")
        
        # 初始化默认的标准位置和移动范围（tool_coordinate_id == 1）
        self._set_parameters_for_tool_coordinate(self.tool_coordinate_id)
    
    def _set_parameters_for_tool_coordinate(self, tool_id: int):
        ...
        if tool_id == 1:
            ...
            # tool_coordinate_id == 2 时的参数
            self.standard_pose = [-117.1344, -344.7371, 215.8233, -179.9815, 0.0051, 90.0711]
            self.x_max = 275.00
            self.x_min = -400.00
            self.y_max = -165.00
            self.y_min = -500.00
            ...
            # 默认参数（tool_coordinate_id == 0 或其他情况）
            self.standard_pose = [-163.0518, -325.5861, 262.9900, -179.9815, 0.0051, 90.0711]
            self.x_max = 270.00
            self.x_min = -390.00
            self.y_max = -195.00
    ...
    def _send_command(self, command: str) -> str:
        ...
        Returns:
            str: 机械臂返回的响应字符串
        """
        if not self.is_connected or self.sock_29999 is None:
    ...
    def set_user_coordinate(self, user_id: int) -> bool:
        """设置用户坐标系
        ...
        try:
            ...
            if response and "error" not in response.lower():
                self.user_coordinate_id = user_id
                self.logger.info(f"✅ 用户坐标系已设置为: {user_id}")
                return True
            else:
                self.logger.error(f"❌ 设置用户坐标系失败: {response}")
                return False
        except Exception as e:
            self.logger.error(f"设置用户坐标系时出错: {e}")
            return False
    
    def set_tool_coordinate(self, tool_id: int) -> bool:
        """设置工具坐标系
        ...
        """
        try:
            command = f"Tool({tool_id})"
            response = self._send_command(command)
            if response and "error" not in response.lower():
                self.tool_coordinate_id = tool_id
                self.logger.info(f"✅ 工具坐标系已设置为: {tool_id}")
                
                # 更新参数以匹配新的工具坐标系
                self._set_parameters_for_tool_coordinate(tool_id)
                
                return True
            else:
                self.logger.error(f"❌ 设置工具坐标系失败: {response}")
                return False
        except Exception as e:
            self.logger.error(f"设置工具坐标系时出错: {e}")
            return False
    ...
    def enable_robot(self) -> bool:
        ...
        if response:
            ...
            self.set_user_coordinate(0)  # 设置用户坐标系为0
            self.set_tool_coordinate(2)  # 设置工具坐标系为2
            return True
        return False
    ...
    def get_pose(self) -> list:
        ...
        """
        # 检查是否使用自定义坐标系
        if USE_CUSTOM_COORDINATE_SYSTEM:
            # 使用指定的用户坐标系和工具坐标系获取位姿
            command = f"GetPose(User={self.user_coordinate_id}, Tool={self.tool_coordinate_id})"
            #self.logger.debug(f"发送坐标获取命令: {command}")
            response = self._send_command(command)
        else:
            # 使用默认方式获取位姿
            self.logger.debug("发送默认坐标获取命令: GetPose()")
        ...
        if "error" in response.lower() or "alarm" in response.lower():
            self.logger.error(f"坐标获取命令返回错误: {response}")
            return []
        ...
        try:
            ...
            if start == -1 or end == -1:
                self.logger.error(f"响应格式错误，未找到坐标数据: {response}")
                return []
            pose_str = response[start+1:end]
            #self.logger.debug(f"解析坐标字符串: '{pose_str}'")
            
            # 检查坐标字符串是否为空
            if not pose_str.strip():
                self.logger.error("坐标字符串为空")
                return []
                
            pose = [float(x.strip()) for x in pose_str.split(',')]
            if len(pose) != 6:
                self.logger.error(f"坐标数据长度错误，期望6个值，实际{len(pose)}个值: {pose}")
                return []
            return pose
        except ValueError as e:
            self.logger.error(f"坐标数据转换失败: {e}")
    ...
    def get_standard_pose(self) -> list:
        """获取标准位置坐标
        
        Returns:
            list: 包含6个元素的列表[X, Y, Z, Rx, Ry, Rz]，表示标准位置
        """
        return self.standard_pose.copy()
    ...
    def move_l(self, x: float, y: float, z: float, rx: float = 180, ry: float = 0, rz: float = 90, wait: bool = True) -> bool:
        ...
        if start_pose is None or len(start_pose) < 3:
            self.logger.error("无法获取有效的起始位置")
            return False
        ...
        if final_pose and len(final_pose) >= 3:
