...
class Aruco_Detector:
    ...
    def __init__(self, marker_length=0.1, aruco_dict_type=aruco.DICT_5X5_100):
        ...
        print(f"  aruco_dict_type: {aruco_dict_type}")
        print(f"  minMarkerPerimeterRate: {self.parameters.minMarkerPerimeterRate}")
        print(f"  maxMarkerPerimeterRate: {self.parameters.maxMarkerPerimeterRate}")
        print(f"  errorCorrectionRate: {self.parameters.errorCorrectionRate}")
    ...
    def detect_markers(self, frame):
        ...
        # 添加调试信息
        if self.printDbgInfo:
            print(f"detectMarkers结果 - corners: {corners}, ids: {ids}")
    ...
    def get_aruco_results(self, corners, ids, mtx, dist, depth_frame=None, intrinsics=None):
        ...
        """
        if self.printDbgInfo:
            print(f"DEBUG: get_aruco_results输入参数 - corners: {corners}, ids: {ids}")
        
        results = []
        
        if ids is None or len(ids) == 0:
            print("DEBUG: ids为None或空，直接返回空结果")
            return results
            ...
            for i in range(len(ids)):
                try:
                    ...
                    if pose_info is not None:
                        ...
                        if self.printDbgInfo:
                            print(f"DEBUG: 成功添加标记 {ids[i][0]} 的位姿信息")
                    else:
                        print(f"DEBUG: 标记 {ids[i][0]} 的位姿信息为None")
                except Exception as e:
            ...
            for i in range(len(ids)):
                try:
                    ...
                    if pose_info is not None:
                        ...
                        results.append(pose_info)
                        if self.printDbgInfo:
                            print(f"DEBUG: 成功添加标记 {ids[i][0]} 的位姿信息（深度方法）")
                    else:
                        print(f"DEBUG: 标记 {ids[i][0]} 的位姿信息为None（深度方法）")
                except Exception as e:
                    # 即使某个标记计算失败，也继续处理其他标记
                    print(f"标记 {ids[i][0]} 计算失败: {e}")
                    continue
        ...
        if self.printDbgInfo:
            print(f"DEBUG: get_aruco_results返回结果 - results: {results}")
        return results
    ...
    def detect_and_process_markers(self, frame, depth_frame=None, draw_results=True, print_results=True):
        ...
        # 添加调试信息，检查detect_markers返回的结果
        if self.printDbgInfo:
            print(f"DEBUG: detect_markers返回结果 - corners类型: {type(corners)}, ids类型: {type(ids)}")
            print(f"DEBUG: detect_markers返回结果 - corners: {corners}")
            print(f"DEBUG: detect_markers返回结果 - ids: {ids}")
        ...
        # 添加调试信息，检查get_aruco_results返回的结果
        if self.printDbgInfo:
            print(f"DEBUG: get_aruco_results返回结果 - markers_info: {markers_info}")
        
        # 检查是否检测到标记
        found_markers = ids is not None and len(ids) > 0
        ...
        # 如果需要绘制结果
        if draw_results and found_markers and len(markers_info) > 0:
            ...
            if  prinInfo:
                ...
                print(f"Debug - len(ids): {len(ids) if ids is not None else 0}")
                print(f"Debug - markers_info: {markers_info}")
                print(f"Debug - len(markers_info): {len(markers_info)}")
                print(f"Debug - found_markers: {found_markers}")
                print(f"Debug - draw_results: {draw_results}")
                print(f"Debug - print_results: {print_results}")
            # 没有检测到标记，但在需要绘制结果的情况下显示提示信息
            cv2.putText(frame, "No markers detected", (20, 40), self.font, 1, (0, 0, 255), 2)
            result['frame'] = frame
        
        # 如果需要打印结果
        if print_results and found_markers and len(markers_info) > 0:
