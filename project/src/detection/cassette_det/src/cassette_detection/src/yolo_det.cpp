...
void render_cassette_with_holes(cv::Mat& img, 
                               const std::vector<CassetteWithHoles>& cassette_res,
                               const std::vector<std::string>& class_names,
                               const std::vector<std::string>& class1_names) 
{
    static const std::vector<cv::Scalar> colors = 
    {
        cv::Scalar(255, 0, 0), cv::Scalar(0, 255, 0), cv::Scalar(0, 0, 255),
    ...
    for(const auto& det : cassette_res)
    {
        // 渲染料盒
        const auto& cassette = det.cassette;
        float x1 = cassette.bbox[0] - cassette.bbox[2] / 2;
        float y1 = cassette.bbox[1] - cassette.bbox[3] / 2;
        float x2 = cassette.bbox[0] + cassette.bbox[2] / 2;
        float y2 = cassette.bbox[1] + cassette.bbox[3] / 2;
        ...
        cv::Rect cassette_rect(x1, y1, x2 - x1, y2 - y1);
        cv::Scalar cassette_color = colors[cassette.class_id % colors.size()];
        cv::rectangle(img, cassette_rect, cassette_color, 2);

        std::string cassette_label;
        if (!class_names.empty() && cassette.class_id < class_names.size()) {
            cassette_label = class_names[cassette.class_id] + ": " + cv::format("%.2f", cassette.conf);
        } else {
            cassette_label = cv::format("%d: %.2f", cassette.class_id, cassette.conf);
        }

        int baseline;
        cv::Size text_size = cv::getTextSize(cassette_label, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &baseline);
        int text_y = std::max(cassette_rect.y, text_size.height + 5);
        cv::rectangle(img, 
                    cv::Point(cassette_rect.x, text_y - text_size.height - 5),
                    cv::Point(cassette_rect.x + text_size.width, text_y),
                    cassette_color, cv::FILLED);
        cv::putText(img, cassette_label, 
                cv::Point(cassette_rect.x, text_y - 5),
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0), 1);

        // 渲染孔洞
        for (const auto& hole : det.holes) {
            float cx = cassette.bbox[0] - cassette.bbox[2] / 2.0f + hole.bbox[0];
            float cy = cassette.bbox[1] - cassette.bbox[3] / 2.0f + hole.bbox[1];
            float w = hole.bbox[2];
            float h = hole.bbox[3];
            ...
            cv::Scalar hole_color = colors[hole.class_id % colors.size()];
            cv::rectangle(img, hole_rect, hole_color, 2);

            std::string hole_label;
            if (!class1_names.empty() && hole.class_id < class1_names.size()) {
                hole_label = class1_names[hole.class_id] + ": " + cv::format("%.2f", hole.conf);
            } else {
                hole_label = cv::format("%d: %.2f", hole.class_id, hole.conf);
            }

            text_size = cv::getTextSize(hole_label, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &baseline);
            text_y = std::max(hole_rect.y, text_size.height + 5);
            cv::rectangle(img, 
                        cv::Point(hole_rect.x, text_y - text_size.height - 5),
                        cv::Point(hole_rect.x + text_size.width, text_y),
                        hole_color, cv::FILLED);
            cv::putText(img, hole_label, 
                    cv::Point(hole_rect.x, text_y - 5),
                    cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0), 1);
        }
    }  
}
