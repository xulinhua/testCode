...
> 说明：该包由 `calib_sim` 复制得到，标定逻辑保持一致，用于 `nova_sim_mujoco` 联调。
...
| `calib_qt_ui_node` | 仅 Qt 订阅 `/calib_sim/*`；需另起标定节点时使用 |
...
colcon build --packages-select calib_sim
...
启动后在窗口中通过下拉框切换 **眼在手外 eth0/eth1**、**眼在手上 eih0/eih1**；参数来自安装目录下的 `share/calib_sim/config/calib_unified.yaml`（各模式带前缀，由 `config_data_manager` 加载）。
...
```bash
ros2 run calib_sim <你的节点> --ros-args --params-file install/calib_sim/share/calib_sim/config/eye_in_hand.yaml
```
...
- 机械臂位姿：`robot_pose_topic`（`calib_sim/msg/ArmPose`）或启用 TF 时由 `tf_base_frame` → `tf_ee_frame_arm*` 查询
- 到达标志：`robot_state_topic`（`std_msgs/Bool`）
- 控制：`/calib_sim/control`（`std_msgs/String`，如 `cmd:set_mode`、`init`、`step` 等，与 Qt/桥接约定一致）

### 发布

- `/calib_sim/status`、`/calib_sim/log`、`/calib_sim/result_text`
- `/calib_sim/raw_image`、`/calib_sim/result_image`（叠加检测与坐标轴）
- `robot_target_topic`：下发采样目标位姿（`ArmPose`）
...
默认写入参数 `output_dir` 下（如工作空间内 `calib_output_mujoco/`），每次 run 带时间戳子目录，包含：
...
仿真下通常先启动 `nova_sim`（Gazebo、桥接、`moveit2_arm_executor`、`/robot_pose` 等），再启动 **`calib_sim_node`**。桥接节点名将 `nova_sim` 侧话题对齐到标定节点期望的名称（见 `nova_sim` README）。
