...
#include "ros_robot_workbench/module/lidar_cam_projection_module.h"

namespace ros_robot_workbench::ui
{

LidarCamProjectionWidget::LidarCamProjectionWidget(QWidget * parent)
: QWidget(parent)
{
  manage::LidarCamProjectionDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("lidar_cam_projection.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("点云投影");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(LidarCamProjectionModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(LidarCamProjectionModuleSummary())]() {
