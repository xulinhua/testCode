...
GraspPoseGenWidget::GraspPoseGenWidget(QWidget * parent)
: QWidget(parent)
{
  manage::GraspPoseGenDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("grasp_pose_gen.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("抓取姿态");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(GraspPoseGenModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(GraspPoseGenModuleSummary())]() {
