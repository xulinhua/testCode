...
#include "ros_robot_workbench/module/detection_overlay_module.h"

namespace ros_robot_workbench::ui
{

DetectionOverlayWidget::DetectionOverlayWidget(QWidget * parent)
: QWidget(parent)
{
  manage::DetectionOverlayDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("detection_overlay.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("检测叠加");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(DetectionOverlayModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(DetectionOverlayModuleSummary())]() {
