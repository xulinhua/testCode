...
#include "ros_robot_workbench/module/path_compare_module.h"

namespace ros_robot_workbench::ui
{

PathCompareWidget::PathCompareWidget(QWidget * parent)
: QWidget(parent)
{
  manage::PathCompareDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("path_compare.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("路径对比");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(PathCompareModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(PathCompareModuleSummary())]() {
