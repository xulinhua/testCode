...
#include "ros_robot_workbench/module/nav2_panel_module.h"

namespace ros_robot_workbench::ui
{

Nav2PanelWidget::Nav2PanelWidget(QWidget * parent)
: QWidget(parent)
{
  manage::Nav2PanelDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("nav2_panel.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("Nav2状态");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(Nav2PanelModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(Nav2PanelModuleSummary())]() {
