...
#include "ros_robot_workbench/module/stereo_calibration_module.h"
...
: QWidget(parent)
{
  ...
  for (const auto & def : StereoOnlineFieldDefs()) {
  ...
  for (const auto & def : StereoOfflineFieldDefs()) {
  ...
  QLineEdit * output_yaml = new QLineEdit(DefaultCalibrationYamlPath("双目标定"));
  ...
  auto image_node = rclcpp::Node::make_shared("ros_robot_workbench_stereo_preview_" + std::to_string(stamp));
  ...
  QObject::connect(run_btn, &QPushButton::clicked, [=]() {
    ...
    if (!SaveCalibrationYaml(
          output_yaml->text(), "双目标定", "online", rt_image_topic->currentText(),
  ...
  QObject::connect(off_run_btn, &QPushButton::clicked, [=]() {
    ...
    if (!SaveCalibrationYaml(
          output_yaml->text(), "双目标定", "offline", off_bag_dir->text(),
          board_type->currentText(), distortion_model->currentText(),
  ...
  const QString default_cfg = ResolveDefaultConfigYamlPath("stereo_calibration.yaml");
