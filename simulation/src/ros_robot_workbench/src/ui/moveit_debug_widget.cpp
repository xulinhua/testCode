...
#include "ros_robot_workbench/module/moveit_debug_module.h"

namespace ros_robot_workbench::ui
{

MoveitDebugWidget::MoveitDebugWidget(QWidget * parent)
: QWidget(parent)
{
  manage::MoveitDebugDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("moveit_debug.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("MoveIt调试");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(MoveitDebugModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(MoveitDebugModuleSummary())]() {
