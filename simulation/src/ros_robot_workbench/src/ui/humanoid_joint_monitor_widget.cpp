...
#include "ros_robot_workbench/module/humanoid_joint_monitor_module.h"

namespace ros_robot_workbench::ui
{

HumanoidJointMonitorWidget::HumanoidJointMonitorWidget(QWidget * parent)
: QWidget(parent)
{
  manage::HumanoidJointMonitorDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("humanoid_joint_monitor.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("全身关节");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(HumanoidJointMonitorModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(HumanoidJointMonitorModuleSummary())]() {
