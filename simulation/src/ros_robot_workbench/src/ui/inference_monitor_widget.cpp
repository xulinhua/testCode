...
InferenceMonitorWidget::InferenceMonitorWidget(QWidget * parent)
: QWidget(parent)
{
  manage::InferenceMonitorDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("inference_monitor.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("推理监视");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(InferenceMonitorModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(InferenceMonitorModuleSummary())]() {
