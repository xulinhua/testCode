...
#include "ros_robot_workbench/module/foot_contact_monitor_module.h"

namespace ros_robot_workbench::ui
{

FootContactMonitorWidget::FootContactMonitorWidget(QWidget * parent)
: QWidget(parent)
{
  manage::FootContactMonitorDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("foot_contact_monitor.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("足端监视");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(FootContactMonitorModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(FootContactMonitorModuleSummary())]() {
