...
#include "ros_robot_workbench/module/balance_panel_module.h"

namespace ros_robot_workbench::ui
{

BalancePanelWidget::BalancePanelWidget(QWidget * parent)
: QWidget(parent)
{
  manage::BalancePanelDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("balance_panel.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("平衡面板");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(BalancePanelModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(BalancePanelModuleSummary())]() {
