...
#include "ros_robot_workbench/module/depth_analyzer_module.h"

namespace ros_robot_workbench::ui
{

DepthAnalyzerWidget::DepthAnalyzerWidget(QWidget * parent)
: QWidget(parent)
{
  manage::DepthAnalyzerDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("depth_analyzer.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("深度分析");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(DepthAnalyzerModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(DepthAnalyzerModuleSummary())]() {
