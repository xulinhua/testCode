...
#include "ros_robot_workbench/module/rl_policy_monitor_module.h"

namespace ros_robot_workbench::ui
{

RlPolicyMonitorWidget::RlPolicyMonitorWidget(QWidget * parent)
: QWidget(parent)
{
  manage::RlPolicyMonitorDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("rl_policy_monitor.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("RL策略监视");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(RlPolicyMonitorModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(RlPolicyMonitorModuleSummary())]() {
