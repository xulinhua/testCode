...
#include "ros_robot_workbench/module/wheel_calib_module.h"

namespace ros_robot_workbench::ui
{

WheelCalibWidget::WheelCalibWidget(QWidget * parent)
: QWidget(parent)
{
  manage::WheelCalibDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("wheel_calib.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("轮速标定");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(WheelCalibModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(WheelCalibModuleSummary())]() {
