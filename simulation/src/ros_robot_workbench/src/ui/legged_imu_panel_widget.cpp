...
#include "ros_robot_workbench/module/legged_imu_panel_module.h"

namespace ros_robot_workbench::ui
{

LeggedImuPanelWidget::LeggedImuPanelWidget(QWidget * parent)
: QWidget(parent)
{
  manage::LeggedImuPanelDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("legged_imu_panel.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("IMU姿态");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(LeggedImuPanelModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(LeggedImuPanelModuleSummary())]() {
