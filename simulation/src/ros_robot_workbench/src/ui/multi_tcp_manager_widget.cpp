...
#include "ros_robot_workbench/module/multi_tcp_manager_module.h"

namespace ros_robot_workbench::ui
{

MultiTcpManagerWidget::MultiTcpManagerWidget(QWidget * parent)
: QWidget(parent)
{
  manage::MultiTcpManagerDataManager dm;
  dm.SetConfigPath(ResolveDefaultConfigYamlPath("multi_tcp_manager.yaml").toStdString());
  dm.Load();
  ...
  QLabel * title = new QLabel("多TCP管理");
  title->setStyleSheet("font-size: 20px; font-weight: 600; color: #22415b;");
  root->addWidget(title);

  QLabel * hint = new QLabel(MultiTcpManagerModuleSummary());
  ...
  QObject::connect(refresh, &QPushButton::clicked, [log, summary = QString(MultiTcpManagerModuleSummary())]() {
