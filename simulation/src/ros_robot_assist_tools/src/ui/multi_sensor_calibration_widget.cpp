...
MultiSensorCalibrationWidget::MultiSensorCalibrationWidget(QWidget * parent)
: QWidget(parent)
{
  ...
  QLabel * title = new QLabel("多传感器标定");
  ...
  for (const auto & def : MultiSensorOnlineFieldDefs()) {
  ...
  for (const auto & def : MultiSensorOfflineFieldDefs()) {
  ...
  QLineEdit * output_yaml = new QLineEdit(DefaultCalibrationYamlPath("多传感器标定"));
  ...
  auto image_node = rclcpp::Node::make_shared("ros_robot_assist_tools_multi_preview_" + std::to_string(stamp));
  ...
  QObject::connect(run_btn, &QPushButton::clicked, [=]() {
    ...
    if (!SaveCalibrationYaml(
          output_yaml->text(), "多传感器标定", "online", rt_image_topic->currentText(),
  ...
  QObject::connect(off_run_btn, &QPushButton::clicked, [=]() {
    ...
    if (!SaveCalibrationYaml(
          output_yaml->text(), "多传感器标定", "offline", off_bag_dir->text(),
          board_type->currentText(), distortion_model->currentText(),
  ...
  const QString default_cfg = ResolveDefaultConfigYamlPath("multi_sensor_calibration.yaml");
