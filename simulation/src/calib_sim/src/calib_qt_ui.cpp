...
int RunCalibQtUiApp(const std::shared_ptr<CalibQtUiRosNode> & ros_node, int argc, char ** argv)
{
  ...
  const fs::path output_root("/home/hs/testCode/simulation/calib_output_gazebo");
  ...
  QObject::connect(btn_clear_all_data, &QPushButton::clicked, [&]() {
    const auto r = QMessageBox::question(
      ...
      QString::fromUtf8("将永久删除 calib_output_gazebo 下所有 calib_run_* 目录，是否继续？"),
    ...
    std::error_code ec;
    if (!fs::exists(output_root, ec) || ec) {
      appendUiLog("[历史数据] calib_output_gazebo 不存在，跳过删除");
      refreshRunList();
      return;
    }
    int dirs_removed = 0;
