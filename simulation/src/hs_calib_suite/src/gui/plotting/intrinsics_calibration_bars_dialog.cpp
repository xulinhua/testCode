...
IntrinsicsCalibrationBarsDialog::IntrinsicsCalibrationBarsDialog(QWidget *parent)
    : QDialog(parent) {
  auto chrome = setup_intrinsics_dialog(
      this,
      QStringLiteral("Calibration result statistics vs single-shot calibration"),
      QStringLiteral("标定内参与 single-shot 下界逐帧 RMS 对比（Tier4）"),
      900);
...
void IntrinsicsCalibrationBarsDialog::refresh(
  ...
  plot_loader_->start(
      session, IntrinsicsPlotKind::CalibrationBars, backend);
}
