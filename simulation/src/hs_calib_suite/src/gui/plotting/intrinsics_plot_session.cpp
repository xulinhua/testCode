...
bool build_intrinsics_plot_input(
  ...
  if (session.uses_stereo_dual_session()) {
    return build_intrinsics_plot_input_for_side(session, QStringLiteral("left"), out);
  }
...
bool build_intrinsics_plot_input_for_side(
  ...
  if (out == nullptr || !session.is_intrinsics() || !session.uses_stereo_dual_session()) {
    return false;
  }
  const auto &state = session.intrinsics_state_for_side(side);
  out->collector = &state.collector();
  out->profile = core::profile_from_config_map(session.solve_options());
  out->collector_params = state.collector().params();
  out->extras = core::calibration_extras_from_config(session.solve_options());
