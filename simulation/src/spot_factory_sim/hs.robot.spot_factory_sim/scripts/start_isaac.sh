...
# 本地资产：工厂仓库 + Spot + policy
if [[ "${SPOT_SKIP_ASSET_CHECK:-0}" != "1" ]]; then
  if python3 "$ROOT/scripts/download_assets.py" --verify-only; then
    echo "Local assets OK (data/scenes|robots|policies)"
  else
    echo "Local assets incomplete — downloading missing files (curl, skip existing) ..."
...
ENABLE_ARGS=()
if [[ "${SPOT_FACTORY_SIM_ENABLE:-1}" == "1" ]]; then
  ENABLE_ARGS=(--enable hs.robot.spot_factory_sim)
fi

echo "Starting Isaac Sim (ext-folder: $EXT_FOLDER)"
echo "  extension: hs.robot.spot_factory_sim"
echo "  ROS_DOMAIN_ID=$ROS_DOMAIN_ID"
echo "  panel: Hs Robot Spot Factory Sim -> Load -> Play"
# Perf + quieter logs:
# - 物理 200Hz（策略约 50Hz）；限制每帧追赶步数，避免低 FPS 时 spiral
