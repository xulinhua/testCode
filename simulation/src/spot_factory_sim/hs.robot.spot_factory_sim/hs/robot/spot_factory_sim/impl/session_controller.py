...
from ..global_variables import DEFAULT_ROBOT_TYPE, ROBOT_TYPE_CART, SPOT_SPAWN_POS
...
from .ros_io.imu_publisher import ImuPublisher
from .ros_io.joint_state_publisher import JointStatePublisher
from .ros_io.lidar_publisher import LidarPublisher
...
class SessionController:
    ...
    def status_text(self) -> str:
        names = {
            ...
            SessionState.LOADED: "LOADED (Press Play to stream + drive)",
            SessionState.STREAMING: "STREAMING (camera + imu + lidar + cmd_vel + odom/tf/joint_states)",
        }
    ...
    def set_robot_type(self, robot_type: str) -> None:
        self._robot_type = ROBOT_TYPE_CART if robot_type == ROBOT_TYPE_CART else DEFAULT_ROBOT_TYPE
    ...
    def stop_streaming_only(self) -> None:
        ...
        self.imu_publisher.stop()
        self.lidar_publisher.stop()
        self.joint_state_publisher.stop()
        self.odom_tf_publisher.stop()
        if self.scene_loader.is_loaded:
            self.scene_loader.robot_runtime.set_cmd_vel(0.0, 0.0, 0.0)
            self._state = SessionState.LOADED
        else:
            self._state = SessionState.IDLE
    ...
    def set_ui_cmd_vel(self, vx: float, vy: float, wz: float, *, active: bool = True) -> None:
        ...
        if self.scene_loader.is_loaded:
            self.scene_loader.robot_runtime.set_cmd_vel(*self._ui_cmd)
    ...
    def on_timeline_event(self, event) -> None:
        if event.type == int(omni.timeline.TimelineEventType.PLAY):
            if self.scene_loader.is_loaded:
                ...
                self.scene_loader.robot_runtime.mark_needs_init()
    ...
    def _start_streaming(self) -> None:
        ...
        imu_ok = self.imu_publisher.start(self.scene_loader.imu_prim_path, cfg)
        lidar_ok = self.lidar_publisher.start(self.scene_loader.lidar_prim_path, cfg)
        js_ok = self.joint_state_publisher.start(cfg, rt.prim_path)
        chassis = self.scene_loader.body_prim_path or rt.prim_path
        odom_ok = self.odom_tf_publisher.start(
            ...
            imu_xyz_base=self.scene_loader.imu_xyz_base,
            imu_link_quat_xyzw_base=self.scene_loader.imu_link_quat_xyzw_base,
            lidar_xyz_base=self.scene_loader.lidar_xyz_base,
            lidar_link_quat_xyzw_base=self.scene_loader.lidar_link_quat_xyzw_base,
        )
        if cam_ok or imu_ok or lidar_ok or js_ok or odom_ok:
            self._state = SessionState.STREAMING
            self._notify()
            # RenderProduct 可能把 Viewport 切到 front_cam，再拉回俯视
            try:
                spawn = getattr(rt, "_spawn_pos", None)
                if spawn is None:
    ...
    def on_physics_step(self, step_size) -> None:
        ...
        if self._ui_driving:
            self.scene_loader.robot_runtime.set_cmd_vel(*self._ui_cmd)
        self.scene_loader.robot_runtime.on_physics_step(float(step_size))
