...
class UIBuilder:
    ...
    def build_ui(self):
        ...
        robot_frame = CollapsableFrame("Robot", collapsed=False)
        with robot_frame:
            with ui.VStack(style=ui_style, spacing=5, height=0):
                with ui.HStack(height=24, spacing=4):
                    ui.Label("Type", width=70, height=0)
                    self._robot_combo = ui.ComboBox(0, *[label for _, label in ROBOT_TYPE_LABELS])
                    try:
                        self._robot_combo.model.add_item_changed_fn(
                            lambda *_a, **_k: self._on_robot_type_changed()
                        )
                    except Exception:
                        try:
                            self._robot_combo.model.get_item_value_model().add_value_changed_fn(
                                lambda _m: self._on_robot_type_changed()
                            )
                        except Exception:
                            pass
                    ui.Spacer()
                ui.Label(
                    "Choose Spot or Cart before Load. Sensors (cam/IMU/lidar) use the same ROS topics.",
                    word_wrap=True,
                    height=0,
                )
        ...
        with actions_frame:
            with ui.VStack(style=ui_style, spacing=5, height=0):
                ...
                self._buttons["load"] = btn_builder(
                    ...
                    type="button",
                    text="Load",
                    tooltip="Load factory warehouse + selected robot + camera/IMU/lidar",
        ...
        self.frames = [robot_frame, scene_frame, drive_frame, topic_frame, stream_frame, actions_frame]
        self._apply_session_config()

    def _on_robot_type_changed(self) -> None:
        idx = 0
        try:
            ...
            idx = 0
        idx = max(0, min(idx, len(ROBOT_TYPE_LABELS) - 1))
        self._robot_type = ROBOT_TYPE_LABELS[idx][0]
        self._session.set_robot_type(self._robot_type)

    def _selected_robot_type(self) -> str:
        self._on_robot_type_changed()
        return self._robot_type if self._robot_type in (ROBOT_TYPE_SPOT, ROBOT_TYPE_CART) else DEFAULT_ROBOT_TYPE
    ...
    def _on_load(self):
        self._apply_session_config()
        robot_type = self._selected_robot_type()
        self._session.set_robot_type(robot_type)
        self._set_status(f"LOADING ({robot_type})...")
