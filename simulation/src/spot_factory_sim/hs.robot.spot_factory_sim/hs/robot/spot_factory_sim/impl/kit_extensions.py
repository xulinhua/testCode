    ...
    try:
        ...
        for ext_id in extension_ids:
            if not em.is_extension_enabled(ext_id):
                ...
                print(f"[SpotFactorySim] enabled extension: {ext_id}")
        return all(em.is_extension_enabled(ext_id) for ext_id in extension_ids)
    except Exception as exc:
        print(f"[SpotFactorySim] failed to enable {extension_ids}: {exc}")
        return False
...
def ensure_sensor_exts_enabled() -> bool:
    return _enable("isaacsim.sensors.physics", "isaacsim.sensors.rtx")


def ensure_wheeled_robots_enabled() -> bool:
    return _enable("isaacsim.robot.wheeled_robots")
