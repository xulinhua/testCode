...
class SpotRuntime:
    ...
    def spawn(
        ...
        spot._decimation = int(POLICY_DECIMATION)
        spot._dt = float(PHYSICS_DT)
    ...
    def _ensure_policy_loaded(self) -> bool:
        ...
        try:
            ...
            # yaml 里是训练用 500Hz/decimation=10；交互场景改成当前物理步，策略仍约 50Hz
            self._spot._dt = float(PHYSICS_DT)
            self._spot._decimation = int(POLICY_DECIMATION)
            self._policy_loaded = True
            print(
                f"SpotRuntime: policy loaded {self._policy_pt} "
                f"physics={1.0 / PHYSICS_DT:.0f}Hz policy≈{POLICY_CONTROL_HZ}Hz "
                f"(decimation={POLICY_DECIMATION})"
            )
            return True
