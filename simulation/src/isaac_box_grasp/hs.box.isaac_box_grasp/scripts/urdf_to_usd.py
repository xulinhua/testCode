#!/usr/bin/env python3
"""Headless URDF -> USD via Isaac Sim isaacsim.asset.importer.urdf."""
...
def _parse_args() -> argparse.Namespace:
    ...
    default_urdf = os.path.join(root, "robot_urdf", "urdf", "HAD2503-D-DEX1.urdf")
    default_usd = os.path.join(root, "data", "robots", "HAD2503-D-DEX1.usd")
    p = argparse.ArgumentParser(description="Convert URDF to USD using Isaac Sim URDF importer")
    p.add_argument("--urdf", default=default_urdf, help="Absolute path to URDF file")
...
def main() -> int:
    ...
    try:
        ...
        if not os.path.isfile(dest_path):
            print(f"ERROR: USD file was not created: {dest_path}", file=sys.stderr)
            return 1

        stage = Usd.Stage.Open(dest_path)
        if stage is None:
            print(f"ERROR: cannot open exported USD: {dest_path}", file=sys.stderr)
            return 1
